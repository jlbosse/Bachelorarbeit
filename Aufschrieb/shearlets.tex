...
\todo{Abschnitt über die grundsätzlichen Erfordernisse, um eine höherdimensionale Wavelettransformation zu basteln}

\begin{theorem}[admissibility condition]
\label{thm:admissibility_criterion}
    Falls $\psi$ und $G$ gewisse Bedingungen erfüllen, erhalten wir ein reproduzierendes System.
    \begin{equation}
        \Delta(\psi)(\xi) = \int_G |\hat\psi(M^t \xi)|^2 |\det M| \d \lambda(M) = 1
...
\begin{equation}
    \mathcal{S}_f(a,s,t) =
    \left\langle D_{M_{as}}T_t\psi, f
    \right\rangle
    = \int f(t,x) \psi\left(\right) \d t \d x
\end{equation}
